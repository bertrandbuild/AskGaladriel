...
import "./VerifyWork.sol";

interface IVerifyWork {
    function sendMessage(string memory _message) external;
}

contract PersonalAgent {
    ...
    function onOracleOpenAiLlmResponse(
        ...
        if (bytes(_errorMessage).length > 0) {
            ...
            // call the next agent
            address targetAgent = 0x88013633e360d5782dCE646Dd6562E8560730A75;
            IVerifyWork targetAgentContract = IVerifyWork(targetAgent);
            string memory messageToSend = string(abi.encodePacked("You are a data gathering agent that can gather data from the web and return it in a structured and formatted way. Here is your instruction plan : ", response));
            targetAgentContract.sendMessage(messageToSend);
        }
    }
