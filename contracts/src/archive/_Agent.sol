...
contract Agent {
    ...
    // @notice Event for agent-to-agent calls
    event AgentToAgentCall(uint indexed callerRunId, address indexed targetAgent, uint indexed targetRunId);
    ...
    // @notice Calls another agent and waits for its result
    // @param callerRunId The ID of the calling agent's run
    // @param targetAgent The address of the target agent
    // @param query The query to pass to the target agent
    // @param max_iterations The maximum number of iterations for the target agent
    function callAnotherAgent(
        uint callerRunId,
        address targetAgent,
        string memory query,
        uint8 max_iterations
    ) public {
        require(msg.sender == agentRuns[callerRunId].owner, "Only run owner can call another agent");
        require(!agentRuns[callerRunId].is_finished, "Caller run is finished");

        Agent targetAgentContract = Agent(targetAgent);
        uint targetRunId = targetAgentContract.runAgent(query, max_iterations);

        emit AgentToAgentCall(callerRunId, targetAgent, targetRunId);
    }
}